...
[CustomEditor(typeof(MovingPlatformByKey))]
public class MovingPlatformByKeyEditor : Editor
{
    private MovingPlatformByKey m_MovingPlatform;
    ...
    private SerializedProperty m_PlatformKeyProperty;
    ...
    {
        m_PreviewPosition = 0;
        m_MovingPlatform = target as MovingPlatformByKey;

        if(!EditorApplication.isPlayingOrWillChangePlaymode)
            MovingPlatformByKeyPreview.CreateNewPreview(m_MovingPlatform);
        ...
        m_PlatformKeyProperty = serializedObject.FindProperty(nameof(m_MovingPlatform.keys));
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Add Node"))
        {
            ...
            m_PlatformKeyProperty.InsertArrayElementAtIndex(index);
            m_PlatformKeyProperty.GetArrayElementAtIndex(index).stringValue = "";
        }
        ...
        for (int i = 0; i < m_MovingPlatform.localNodes.Length; ++i)
        {
            ...
            EditorGUILayout.PropertyField(m_PlatformWaitTimeProperty.GetArrayElementAtIndex(i), new GUIContent("Wait Time"));
            EditorGUILayout.PropertyField(m_PlatformKeyProperty.GetArrayElementAtIndex(i), new GUIContent("Key"));
        ...
        if (delete != -1)
        {
            ...
            m_PlatformKeyProperty.DeleteArrayElementAtIndex(delete);
        }
    ...
    void MovePreview()
    {
        ...
        MovingPlatformByKeyPreview.preview.transform.position = m_MovingPlatform.transform.TransformPoint(localPos);
