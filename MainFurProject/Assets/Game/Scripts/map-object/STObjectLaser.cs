...
public class STObjectLaser : MonoBehaviour
{
    ...
    [SerializeField] private float m_monsterDealDamageDuration = 0f;

    private List<ParticleSystem> m_listEff;
    private DamageDealerInfo m_infor;
    private float m_monsterDealDamageTimer;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < m_lines.Count; i++)
        {
            ...
            else
            {
                ...
                {
                    m_monsterDealDamageTimer -= Time.deltaTime;
                    STObjectMonster monster = hit.collider.GetComponent<STObjectMonster>();
                    if (monster != null && m_monsterDealDamageTimer <= 0)
                    {
                        m_monsterDealDamageTimer = m_monsterDealDamageDuration;
                        monster.gameObject.SendMessage("OnHit", m_infor, SendMessageOptions.DontRequireReceiver);
                    }
                }
