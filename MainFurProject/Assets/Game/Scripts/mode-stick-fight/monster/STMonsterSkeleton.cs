...
public class STMonsterSkeleton : STObjectMonster
{
    [Header("Config")]
    ...
    void ThrowBone()
    {
        ...
        if(xDistance < m_minAttackRange)
        {
            targetPos += new Vector2(m_direction * m_minAttackRange, 0);
        }
        Vector2 finalVelocity = Blobcreate.ProjectileToolkit.Projectile.VelocityByHeight(bone.transform.position, targetPos, m_maxBulletHeight);
